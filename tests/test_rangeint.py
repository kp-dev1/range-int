...
def test_neg_clamps():
    assert (-RangeInt(75, 0, 100)).value == 0  # -75 < min=0
...
# ── Callbacks (Refined System) ────────────────────────────────────────────────

def test_on_percentage_return_self():
    r = RangeInt(50, 0, 100)
    assert r.on_percentage(0.5, lambda: None) is r

def test_callback_directional_up():
    fired = []
    r = RangeInt(0, 0, 100)
    r.on_percentage(0.5, lambda: fired.append("up"), directional="up", through=True)
...
def test_callback_directional_down():
    fired = []
    r = RangeInt(100, 0, 100)
    r.on_percentage(0.5, lambda: fired.append("down"), directional="down", through=True)
...
def test_callback_through_true_tripwire():
    fired = []
    r = RangeInt(0, 0, 100)
    r.on_percentage(0.5, lambda: fired.append("hit"), through=True)
    
    r.value = 90
    assert "hit" in fired

def test_callback_through_false_bullseye_miss():
    fired = []
    r = RangeInt(0, 0, 100)
    r.on_percentage(0.5, lambda: fired.append("hit"), through=False)
    
    r.value = 60
    assert "hit" not in fired

def test_callback_through_false_bullseye_hit():
    fired = []
    r = RangeInt(0, 0, 100)
    r.on_percentage(0.5, lambda: fired.append("hit"), through=False)
...
def test_float_precision_bullseye():
    fired = []
    # Test float division tolerance
    r = RangeInt(0, 0, 3) 
    r.on_percentage(0.333333333, lambda: fired.append("hit"), through=False)
    
    r.value = 1 
    assert "hit" in fired

def test_initial_value_no_callback():
    fired = []
    r = RangeInt(50, 0, 100)
    r.on_percentage(0.5, lambda: fired.append("hit"))
    assert fired == []
